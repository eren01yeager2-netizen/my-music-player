    ...
    for (var path in searchPaths) {
      ...
      if (await dir.exists()) {
        try {
          ...
          for (var f in files) {
            if (f.path.endsWith('.mp3') || f.path.endsWith('.m4a')) {
              String name = f.path.split('/').last.replaceAll('.mp3', '').replaceAll('.m4a', '');
              temp.add(SongInfo(title: name, artist: "Local Audio", path: f.path));
            }
          }
        } catch (_) {}
      }
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      ...
                      const Text("Offline downloaded songs", style: TextStyle(color: Colors.black87, fontSize: 12)),
                      const SizedBox(height: 14),
                      CircleAvatar(
                        radius: 20,
        ...
        children: [
          ...
          Expanded(
            child: _songs.isEmpty
                ? const Center(child: Text("No offline MP3 files found in Music or Download folder.", style: TextStyle(color: Colors.white54)))
                : ListView.builder(
